...
def upload_document():
    try:
        ...
        knowledge_base_by_name = KnowledgeBase.query.filter_by(name=name, user_id=user_id).first()
        if knowledge_base_by_name:
            return jsonify({"error": "A knowledge base with this name already exists"}), 402
