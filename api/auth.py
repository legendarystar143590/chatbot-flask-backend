...
def login():
    ...
    try:
        ...
        if check_password_hash(user.password, password):
            ...
            return jsonify({'accessToken': access_token, 'userId':user.id, 'userIndex':user.index, 'firstName':user.first_name, 'lastName':user.last_name, 'role':user.role, 'status':user.status}), 200
